...
public:
	...
	BOOL JoinSecondChannel(LPCTSTR lpChannelName, UINT nUID = 0, LPCSTR lpDynamicKey = NULL);
	BOOL LeaveChannel();
	BOOL LeaveSecondChannel();
	CString GetChanelName();
	CString GetSecondChannelName();
	CString GetCallID();
	...
	BOOL EnableScreenCapture(HWND hWnd, int nCapFPS = 15, LPCRECT lpCapRect = NULL, BOOL bEnable = TRUE,int nBitrate = 1000);
...
private:
	...
	CString     m_strSecondChannelName;
	BOOL		m_bVideoEnable;
	BOOL		m_bAudioEnable;
...
public:
	...
	static CAGChannelEventHandler m_ChannelEventHandler;
};
