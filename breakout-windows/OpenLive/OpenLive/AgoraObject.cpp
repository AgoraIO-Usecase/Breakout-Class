...
CAGChannelEventHandler CAgoraObject::m_ChannelEventHandler;
...
void CAgoraObject::SetMsgHandlerWnd(HWND hWnd)
{
	...
	m_ChannelEventHandler.SetMsgReceiver(hWnd);
}
...
BOOL CAgoraObject::JoinSecondChannel(LPCTSTR lpChannelName,UINT nUID /*= 0*/, LPCSTR lpDynamicKey /*= NULL*/)
{
	int nRet = 0;

	//CAGEngineEventHandler* lpSubHandle = new CAGEngineEventHandler;
	LPCSTR lpStreamInfo = "{\"owner\":true,\"width\":640,\"height\":480,\"bitrate\":500}";
	CHAR szChannelName[128];

	::WideCharToMultiByte(CP_ACP, 0, lpChannelName, -1, szChannelName, 128, NULL, NULL);
	if (m_channel != NULL) {
		if (strcmp(m_channel->channelId(), szChannelName) != 0) {
		//release old channel
			m_channel->release();
			m_channel = NULL;
		}
	...
	if (m_channel == NULL) {
		//create channel
		 m_channel = static_cast<IRtcEngine2 *>(m_lpAgoraEngine)->createChannel(szChannelName);
		if (m_channel == NULL) {
			return false;
		}
		m_channel->setChannelEventHandler(&m_ChannelEventHandler);
		m_strSecondChannelName = lpChannelName;
...
BOOL CAgoraObject::LeaveSecondChannel()
{
	if (m_channel != NULL) {
		int nRet = m_channel->leaveChannel();
		return nRet == 0 ? TRUE : FALSE;
	}
...
CString CAgoraObject::GetSecondChannelName()
{
	return m_strSecondChannelName;
}
